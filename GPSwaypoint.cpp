...
int main(int argc, char** argv)
{
	...
	sleep(6);
 	...
 	wp_alt[4]=2;
 	wp_heading[4]=180;
 	set_destination(1,0,3,0);
 	sleep(5);
 	set_destination(1,1,3,0);
 	sleep(5);
 	set_destination(0,1,3,0);
 	sleep(5);
	...
	while(ros::ok())
	{
		...
		rate.sleep();
   		...
   		ROS_INFO("distance=%f",get_two_points_distance(wp_lat[p],wp_lon[p],get_current_lat(),get_current_lon()));
		if(calculateDistance(wp_lat[p],wp_lon[p],get_current_lat(),get_current_lon())<1)
		{
			if(p<=4) 
			{
