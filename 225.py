...
int main(int argc, char** argv)
{
	...
	while(ros::ok())
	{
		...
		{  
				...
				if (desire_xyVel_[1]>1){//limit velocity
					desire_xyVel_[1]=1;
				...
				APMLand.send_body_velxyz_setpoint(desire_xyVel_,desire_yawVel_);
         		  ...
         		  if((abs(t_ca[0])<0.2)&&(abs(t_ca[1])<0.2)&&detect_state==true)
        		  {
					ROS_INFO("countdown");
        		  	if(countonlytime){
